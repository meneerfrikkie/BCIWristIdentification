clc
ExperimentName = "Exp_LDA_ANOVA_BayesianOptimization";
...
for p = 1:length(PatientIDs)
    for c = 1:length(ChosenTableStrings)
        ...
        for f = 1:length(highestAccuraciesNumberFeatures)
                    ...
                    % CSV Storing
                    % Reshape storedPredictorNames into a column vector
                    storedPredictorNamesColumn = storedPredictorNames(:);
                    
                    % Add a header for the CSV file
                    header = {'StoredPredictorNames'};
                    
                    % Combine the header and the data into a cell array
                    dataToWrite = [header; storedPredictorNamesColumn];
                    
                    % Write the data to a CSV file
                    writecell(dataToWrite, fullFilePathCSV);

                    % Save the data to a .mat file
                    save(fullFilePath, 'storedPredictorNamesColumn');
