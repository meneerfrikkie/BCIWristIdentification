...
function Exp1_SVM_ANOVA(ChannelPairNumber, DataName,startingNumberofFeatures,stepsize,totalNumberofFeatures,countedTimeSlots, numberFolds)
ExperimentName = sprintf("Exp%d_LinearSVM_ANOVA_ChannelPair%d_%s_SlidingWindow",numberFolds,ChannelPairNumber,DataName);
Data  = DataName; 
...
countTimeSlot = 0;
countChannels = countedTimeSlots;
...
for p = 1:length(PatientIDs)
    for c = 1:length(ChosenTableStrings)
        clearvars -except PatientIDs ChosenTableStrings ExperimentName countTimeSlot countChannels ChannelPair numberFolds startingNumberofFeatures stepsize  totalNumberofFeatures Data p c
