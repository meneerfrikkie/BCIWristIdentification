...
HyperparameterForSelectedFeatures = {};
...
for p = 1:length(PatientIDs)
     ...
     %Hyperparameter tuning 
            ...
            for k = 1:length(matFiles)
                % Get the full file path
                fileName = matFiles(k).name;
                fullFilePath = fullfile(TablePathForHyperparameterTuning, fileName);
                
                % Load the .mat file
                fileDataHyperparameterTuning = load(fullFilePath);
    
                HyperparameterForSelectedFeatures{end+1,1} = fileDataHyperparameterTuning; 
            end
...
dataTable = table(TablePatientIds, ...
                  ...
                  'VariableNames', {'PatientID', ...
                                   ...
                                   'Hyperparameters for Selected Features', ...
                                   'Mean Accuracy', ...
                                   'Mean Precision', ...
                                   'Mean Recall', ...
                                   'Mean F1-Score', ...
