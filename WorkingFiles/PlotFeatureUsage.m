function PlotFeatureUsage(matFile, minCount)
    ...
    original_time_range = time_range - time_shift;
    ...
    % Get the folder path from the .mat file
    [filePath, ~, ~] = fileparts(matFile);
    
    % Define the filename for the saved figure, including minCount to make it unique
    saveFileName = fullfile(filePath, ['FeatureUsageOverTime_MinCount_' num2str(minCount) '.png']);
    
    % Save the figure as a .png in the same folder as the .mat file
